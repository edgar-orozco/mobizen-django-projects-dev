...
from drivemee import models, utilities, stats
...
def notify_unpaid_solicitud():
    ...
    recibos = models.Recibo.objects.filter(Q(status='pendiente') & Q(timestamp__lte=pending_time) & Q(notified=False))
    for recibo in recibos:
        ...
        recibo.save()
        async_push.delay(deviceToken=solicitud.client.deviceToken, alert=u'Realiza el pago del Servicio a Domicilio para el auto '+vehiculo+u' para no perder tu cita', push_time='now', action='verifica.showValet', token=solicitud.solicitudToken.deviceToken, sound=' ')
    return len(recibos)
...
def purge_unpaid_solicitud():
    ...
    recibos = models.Recibo.objects.filter(Q(status='pendiente') & Q(timestamp__lte=pending_time))
    for recibo in recibos:
        solicitud = recibo.solicitud
        ...
        if solicitud.vehiculo:
            ...
            vehiculo = solicitud.placa
        recibo.delete()
        async_push.delay(deviceToken=solicitud.client.deviceToken, alert=u'Servicio de Valet para el auto '+vehiculo+u' ha sido cancelado por falta de pago', push_time='now', action='verifica.showValet', token=solicitud.solicitudToken.deviceToken, sound=' ')
    return len(recibos)
