...
class SolicitudSerializer(serializers.ModelSerializer):
    ...
    cupon = serializers.SlugRelatedField(slug_field='codigo', queryset=models.Cupon.objects.all())
    documentos = DocumentosSerializer(read_only=True)
    costo = serializers.ReadOnlyField(source='costo_real')
    recibo_timestamp = serializers.ReadOnlyField(source='recibo.timestamp_pagado')
    recibo_status = serializers.ReadOnlyField(source='recibo.status')
    class Meta:
        model = models.Solicitud
        fields = ('folio', 'solicitudToken', 'telefono', 'email', 'nombre', 'placa', 'marca', 'submarca', 'modelo','vehiculo', 'cupon', 'direcciones', 'cita', 'operador', 'evaluacion', 'status', 'documentos', 'ultimo_holograma', 'costo', 'recibo_timestamp', 'recibo_status')
...
class SolicitudInsertSerializer(serializers.Serializer):
    ...
    force_payment = serializers.BooleanField(required=False)
